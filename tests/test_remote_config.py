...
IMPORTANT: When run via pytest, this file only exposes test_remote_config() which
invokes the file as a subprocess. The sys.modules mocking below only runs in the
subprocess, preventing pollution of other tests.

Run standalone with: python3 tests/test_remote_config.py
"""
...
def test_remote_config():
    """Run remote config integration test in subprocess to avoid sys.modules pollution.

    This test mocks bittensor/aurelius in sys.modules which would poison
    every subsequent test if run in-process. Running as a subprocess isolates
    the pollution completely.
    """
    ...
    assert result.returncode == 0, (
        f"Remote config test failed:\nSTDOUT:\n{result.stdout}\nSTDERR:\n{result.stderr}"
    )


# ---------------------------------------------------------------------------
# Everything below runs ONLY when executed as a script (subprocess).
# This prevents sys.modules pollution when pytest imports this file.
# ---------------------------------------------------------------------------
