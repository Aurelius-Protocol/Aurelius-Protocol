...
class TestResponseModeration:
    """Verify that LLM-generated responses are moderated before scoring."""

    @patch("aurelius.validator.experiments.moral_reasoning.experiment.call_chat_api_with_fallback")
    @patch("aurelius.validator.experiments.moral_reasoning.judge.call_chat_api_with_fallback")
    def test_response_moderation_flagged_rejects(self, mock_judge_call, mock_response_call):
        """If the generated response is flagged, submission is rejected."""
        core = _make_mock_core()

        # Response generation succeeds
        mock_response_call.return_value = (
            _mock_llm_response("A harmful response..."),
            "deepseek-v3",
        )

        # Moderation: scenario passes, but response is flagged
        call_count = [0]
        def side_effect_moderate(text):
            call_count[0] += 1
            result = MagicMock()
            if call_count[0] == 1:
                # First call: scenario moderation — not flagged
                result.flagged = False
            else:
                # Second call: response moderation — flagged
                result.flagged = True
            return result

        core.moderation_provider.moderate.side_effect = side_effect_moderate

        experiment = _make_experiment(core)
        synapse = MagicMock()
        synapse.prompt = "A dilemma scenario..."
        synapse.experiment_id = "moral-reasoning"
        synapse.dendrite.hotkey = "miner_hotkey_123"

        result = experiment._handle_scenario(synapse)

        assert result.danger_score == 0.0
        assert result.accepted is False
        assert "response" in result.rejection_reason.lower()
        # Judge should NOT be called
        mock_judge_call.assert_not_called()

    @patch("aurelius.validator.experiments.moral_reasoning.experiment.call_chat_api_with_fallback")
    @patch("aurelius.validator.experiments.moral_reasoning.judge.call_chat_api_with_fallback")
    def test_response_moderation_error_fails_closed(self, mock_judge_call, mock_response_call):
        """If response moderation errors, fail closed (reject)."""
        core = _make_mock_core()

        mock_response_call.return_value = (
            _mock_llm_response("A response..."),
            "deepseek-v3",
        )

        # Scenario moderation passes, response moderation throws
        call_count = [0]
        def side_effect_moderate(text):
            call_count[0] += 1
            if call_count[0] == 1:
